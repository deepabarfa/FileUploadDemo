...
//when data insert into order1 table when data delete from addtocart table
	...
	public int dispatchProduct(String productID, int status) 
	{
		int y=0;
		   try {
			   Connection con=new LoginConnect().start();
			   	  PreparedStatement ps=con.prepareStatement("update order1 set status=? where productID=?");//placeholder
			   	    ps.setInt(1,status);
	...
	public int cancelProduct(String productID, int status) //create function
	{
		int y=0;
		   try {
	...
	try {	
	    ...
	    PreparedStatement ps=con.prepareStatement("select * from order1 where status='1'");//query
		ResultSet rs= ps.executeQuery();// method that return a Resultset object which contains the result returned by the execution query
	while(rs.next())
	{
		Zeppy e=new Zeppy();
